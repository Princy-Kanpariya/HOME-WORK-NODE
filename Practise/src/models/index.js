module.exports.User = require("./user.model");
module.exports.Category = require("./category.model");
module.exports.Product = require("./product.model");
module.exports.Book = require("./book.model");
// module.exports.Token = require("./token.model");
module.exports.Bus = require("./bus.model");
module.exports.Hotel = require("./hotel.model");
module.exports.Stationary = require("./stationary.model");
module.exports.School = require("./school.model"); //student id,name,std
module.exports.Travel = require("./travel.model");
module.exports.Pharmacy = require("./pharmacy.model"); //medicine id,name,type,exp,mfg,mrp
module.exports.Jwellery = require("./jwellery.model"); //customer id,name,piece
module.exports.Movie = require("./movie.model");
module.exports.Music = require("./music.model");
module.exports.Grocery = require("./grocery.model"); //veg name,quantity,quality
module.exports.Mobile = require("./mobile.model");
